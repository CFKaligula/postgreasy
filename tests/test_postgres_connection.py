...
def test_create_table():
    with PostgresConnection(database='ovviadatatransaction_test') as conn:
        ...
        conn.create_table_if_not_exists('public', table_name, sql.SQL('x int, y text'))
        assert conn.check_if_table_exists(schema_name, table_name)

        conn.execute_query_on_db(sql.SQL('drop table {schema}.{table}').format(schema=sql.Identifier(schema_name), table=sql.Identifier(table_name)))
        assert not conn.check_if_table_exists(schema_name, table_name)
...
def test_create_schema():
    with PostgresConnection() as conn:
        ...
        conn.create_schema_if_not_exists(name)
        ...
        result = conn.fetch_with_query_on_db(exist_query)

        delete_query = sql.SQL('drop schema {schema}').format(schema=sql.Identifier(name))
        conn.execute_query_on_db(delete_query)
